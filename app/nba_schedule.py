...
league_schedule = LeagueSchedule(month_schedule=[month['mscd'] for month in schedule['lscd']])
...
schedule_db_again = notion.client.databases.query(database_id=SCHEDULE_DATABASE_ID, start_cursor=schedule_db['next_cursor'])
...
def clear_db_in_future():
    ...
    for row in database_rows:
      ...
      if 'NBA' in row['properties']['League']['rich_text'][0]['plain_text'] and game_date > beginning_of_today:
...
def recursive_fetch_and_delete(next_cursor=None):
  database_response = cast(Any, notion.client.databases.query(database_id=SCHEDULE_DATABASE_ID, next_cursor=next_cursor))
...
def format_date(game: Game) -> str:
  return game.eastern_time.strftime('%Y-%m-%dT%H:%M:%S') # todo fix this from saying UTC in Notion
  ...
  for game in month.games:
    if game.eastern_time > beginning_of_today:
      for broadcaster in game.broadcasting.broadcast:
        if broadcaster.type == 'tv' and broadcaster.scope == 'natl' and broadcaster.disp not in BROADCASTER_BADLIST:
          pp(game.game_date)
          ...
          notion.client.pages.create(parent={"database_id": SCHEDULE_DATABASE_ID}, properties=props)
